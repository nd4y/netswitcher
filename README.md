...
На вкладке «Профили» у каждой Wi-Fi-сети есть кнопка «Поделиться» — тремя способами:
...
* **NFC-метка** — запись учётных данных на перезаписываемую метку в формате
  ...
  предложение подключиться. Прямая передача «телефон→телефон» (Android Beam) удалена из
  Android 10 и в системе больше недоступна, поэтому остаётся запись на метку.
...
Панель закрывается тапом мимо карточки, крестиком или кнопкой «Назад».
...
порядок общий с вкладкой «Профили», так что перетаскивание в любом из двух мест
двигает карточку в обоих. Одноразовые действия (`Wi-Fi on`, `LTE only`, `Ethernet only`)
на главный экран не выводятся — им место на виджете, ярлыке или плитке.
...
CI прогоняет юнит-тесты и собирает debug и release APK на каждый push в `main`
(`.github/workflows/android.yml`), артефакты — во вкладке Actions. Тег `v*`
публикует GitHub Release с единственным APK — так Obtainium не гадает, какой
файл ему брать.

### Подпись релизов

Obtainium обновляет приложение поверх установленного, поэтому подпись должна быть
одна и та же от версии к версии. Сгенерируйте ключ:

```bash
keytool -genkeypair -keystore netswitcher-release.jks -alias netswitcher -keyalg RSA -keysize 4096 -validity 10000
```

и добавьте в Settings → Secrets and variables → Actions четыре секрета:
`KEYSTORE_BASE64` (файл ключа в base64), `KEYSTORE_PASSWORD`, `KEY_ALIAS`,
`KEY_PASSWORD`. Сборка по тегу без этих секретов намеренно падает с ошибкой —
неподписанный APK всё равно нельзя ни установить, ни обновить.

Ключ подписи невосстановим: потеряете — обновления для уже установивших сломаются.

Локально подпись включается теми же значениями через переменные окружения
`NETSWITCHER_KEYSTORE`, `NETSWITCHER_KEYSTORE_PASSWORD`, `NETSWITCHER_KEY_ALIAS`,
`NETSWITCHER_KEY_PASSWORD`. Без них release-сборка остаётся неподписанной.
...
Стек: Gradle 8.11.1, AGP 8.9.1, Kotlin 2.0.21, compileSdk/targetSdk 36, minSdk 31.
Требуется JDK 17 и Android SDK 36.
